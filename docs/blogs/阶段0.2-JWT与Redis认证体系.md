...
```mermaid
flowchart TD
    B["浏览器前端"] --> AT["本地保存 Access Token"]
    B --> RT["Cookie 保存 Refresh Token"]
    B --> CSRF["Cookie 保存 CSRF Token"]
    AT -->|访问业务接口| JWT["Flask 校验 JWT"]
    RT -->|刷新访问凭证| JWT
    CSRF -->|保护刷新请求| JWT
    JWT -->|检查登录状态| REDIS["Redis 登录会话与吊销记录"]
    JWT --> LOGIN["登录校验"]
    LOGIN -->|加载最新用户| MYSQL["MySQL 用户角色与资源归属"]
    LOGIN --> API["聊天会话与 SSE 接口"]
    LOGIN --> ADMIN["管理员校验"]
    ADMIN -->|读取最新角色| MYSQL
    ADMIN --> API
```
...
```mermaid
flowchart TD
    F["浏览器提交登录请求"] --> A["Flask 认证接口"]
    A --> M["MySQL 查询用户并校验密码"]
    M --> TOKEN["生成登录会话和两类 Token"]
    TOKEN --> R["Redis 保存登录会话与刷新标识"]
    R --> RESULT["返回 Access Token 和 Refresh Cookie"]
    RESULT --> STORE["前端保存 Access Token"]
```
...
```mermaid
flowchart TD
    REQUEST["前端请求业务接口"] --> EXPIRED["Access Token 已过期"]
    EXPIRED --> REFRESH["携带 Refresh Cookie 和 CSRF Token"]
    REFRESH --> REDIS["Redis 校验登录会话和刷新标识"]
    REDIS --> MYSQL["MySQL 加载最新用户角色"]
    MYSQL --> ROTATE["原子轮换刷新标识"]
    ROTATE --> TOKEN["返回新的 Access Token 和 Refresh Cookie"]
    TOKEN --> RETRY["前端自动重试原请求"]
```
...
```mermaid
flowchart TD
    A["Access Token 被窃取"] --> B["判断是否已经发现风险"]
...
```mermaid
flowchart TD
    FIRST["前端使用旧 Refresh Token"] --> CHECK["Redis 检查当前刷新标识"]
    CHECK -->|匹配| ROTATE["原子替换为新刷新标识"]
    ROTATE --> RETURN["返回新 Access Token 和 Refresh Token"]
    RETURN --> REUSE["再次提交旧 Refresh Token"]
    REUSE --> CHECK_AGAIN["Redis 再次检查刷新标识"]
    CHECK_AGAIN -->|不匹配| REJECT["拒绝旧 Refresh Token"]
```
...
```mermaid
flowchart TD
    A["请求携带签名正确的 JWT"] --> B["检查 Redis 是否可用"]
    B -->|是| C["检查登录状态并继续"]
    B -->|否| D["拒绝认证请求"]
    D --> E["健康检查显示 degraded"]
```
