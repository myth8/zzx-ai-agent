...
Redis 保存登录会话与刷新标识
        │
        ▼
返回 Access Token 和 Refresh Cookie
        │
        ▼
前端保存 Access Token
```
                 ...
                 └─→ 有效：从 MySQL 加载最新用户
                         ...
                         检查用户是否存在
                           ...
                           └─→ 存在：进入业务接口
                                         │
                                         ▼
                         使用 user_id 和 session_id 校验资源归属
```
...
Access Token 已过期
        │
        ▼
携带 Refresh Cookie 和 CSRF Token
        │
        ▼
Redis 校验登录会话和刷新标识
        │
...
原子轮换刷新标识
        │
        ▼
返回新的 Access Token 和 Refresh Cookie
        │
        ▼
前端自动重试原请求
```
...
判断是否已经发现风险
  ...
  └─→ 尚未发现
      ...
      无法主动吊销
         │
         ▼
      Access Token 最多使用到短期过期
         │
         ▼
      攻击者仍需获得 HttpOnly Refresh Token
```
...
原子替换为新刷新标识
        │
        ▼
返回新 Access Token 和 Refresh Token
        │
        ▼
再次提交旧 Refresh Token
        │
...
Access Token 目前仍保存在 localStorage，以兼容现有 Bearer Token 和 SSE 调用方式。阶段 0.3 已完成 CSP、Markdown-It、DOMPurify 和危险 URL 协议白名单，但 localStorage 在发生其他 XSS 漏洞时仍可被读取，后续仍可进一步评估改为仅保存在内存中。
...
5. 已登录用户可以兑换管理员邀请码。
6. 过期、错误签名和错误 audience 的 JWT 均被拒绝。
7. 两个 SSE 聊天入口未登录访问均返回统一 401。
8. 用户 A 无法读取、重命名或删除用户 B 的会话。
9. 注册、登录账号、聊天和管理员写操作的 Redis 限流。
...
2. 继续评估刷新接口的独立限流；登录、注册、聊天、邀请码兑换和 RAG 管理限流已经在阶段 0.3 完成。
