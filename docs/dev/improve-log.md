...
## 2026-09-16：RAG 切片元信息展示与元数据软过滤

### 本次目标

让管理员可以查看切片真实保存的完整元信息，并在不牺牲全库召回兜底能力的前提下，使用文档标题和主题降低跨场景无关召回。

### 前端切片元信息

- 切片卡片保留文档标题、章节标题、切片 ID 和内容指纹等常用快速标签。
- 新增可展开的“全部切片元信息”区域，直接展示切片表 `metadata` 字段返回的全部内容，不由前端重新生成数据。
- 每个字段标注文档继承、标题提取或切片生成来源。
- 补充 `source`、`chunk_index`、`char_count` 等字段中文名称，并支持数组、JSON 对象和长值格式化展示。
- 元信息区域在窄屏下自动切换为单列布局。

### title 与 topics 软过滤

- 查询前对用户问题与 `title`、`topics`、兼容字段 `topic` 做保守的明确包含匹配。
- 标题匹配会去除“恋爱常见问题”“篇”等通用文字，避免所有文档因为共同前缀被同时命中。
- `topics` 支持 YAML 数组被转换为 Chroma 标量后的 JSON 字符串格式。
- 命中元数据时，以源文档为单位得到共同候选切片；Chroma 使用 `source` 条件过滤，BM25 在相同切片集合上执行检索。
- 未命中元数据时不猜测关系阶段或主题，Chroma 和 BM25 都保持原有全库召回。
- 元数据只约束候选范围，不修改向量相似度、BM25 分数、RRF 权重或 Reranker 分数。

### 一致性设计

元数据预过滤必须同时应用于两路召回。只过滤 Chroma、却让 BM25 继续检索全库，会使无关文档重新进入 RRF 融合结果；只在 BM25 全库结果返回后再过滤，也可能因为 Top K 已经被其他文档占满而漏掉正确切片。因此，当前方案在召回前先确定候选切片集合，再分别执行两种检索。

### 验证结果

- 前端生产构建通过。
- 新增标题命中、主题命中、无命中全库兜底测试。
- 新增 Chroma 与 BM25 使用同一过滤候选集的回归测试。
- RAG 定向测试 4 项全部通过。
